...
def configure_replica(values, deploy_config):
    ...
    for service_name, config in deploy_config["services"].items():
        ...
        if service_name == "llm":
            ...
            replicas.append({
                "name": engine,
                "replicaCount": config["instance_num"]
            })
        else:
            replicas.append({
                "name": service_name,
...
def get_output_filename(num_nodes, with_rerank, example_type):
    """Generate output filename based on configuration."""
    rerank_suffix = "with-rerank-" if with_rerank else ""
    return f"{example_type}-{num_nodes}-gaudi-{rerank_suffix}values.yaml"
...
def configure_models(values, deploy_config):
    ...
    for service_name, config in deploy_config["services"].items():
        ...
        if service_name == "llm":
            ...
            # For all other services with model_id
            if service_name not in values:
                values[service_name] = {}
            values[service_name]["model"] = {
                "name": config["model_id"]
            }
            
    return values

def generate_helm_values(example_type, node_selector=None, deploy_config=None):
    ...
    filename = get_output_filename(num_nodes, with_rerank, example_type)
